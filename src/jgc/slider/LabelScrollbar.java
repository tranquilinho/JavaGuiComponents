 ...
 * - current index label (with an optional descriptive prefix) 
 */
 ...
 *  e-mail address 'jcuenca@cnb.csic.es'
 ***************************************************************************/
...
public class LabelScrollbar extends JPanel implements AdjustmentListener,
		Adjustable {

	private static final long serialVersionUID = -8309565724289625345L;

	// extent is required by JScrollBar
	private static int extent = 20;

	private JLabel textLabel, minLabel, maxLabel;
	private String text = ""; // optional text to clarify the value shown in
								// textLabel
	...
	public LabelScrollbar(int min, int max) {
		// JScrollbar range is minimum..maximum+extent (instead of a simple
		// minimum..maximum)
	...
	public void setMaximum(int n) {
		...
		if (maxLabel != null)
			maxLabel.setText(htmlColor(String.valueOf(n), DEFAULT_TEXT_COLOR,
					true));
	}
	...
	public void setMinimum(int n) {
		...
		if (minLabel != null)
			minLabel.setText(htmlColor(String.valueOf(n), DEFAULT_TEXT_COLOR,
					true));
	}
	...
	private void fireAdjustmentValueChanged(int id, int type, int value,
		...
		for (int i = listeners.length - 2; i >= 0; i -= 2) {
			if (listeners[i] == AdjustmentListener.class) {
				...
				((AdjustmentListener) listeners[i + 1])
	...
	/**
	 * @return value as string with filling zeroes (so the component does not
	 *         need to resize when changing tenths or hundreds)
	 */
	...
	private void updateTextLabel() {
		textLabel.setText("<html>"
				+ htmlColor(getText(), DEFAULT_TEXT_COLOR, false)
				+ htmlColor(getStringValue(), HIGHLIGHTED_TEXT_COLOR, false)
				+ "</html>");
	}
	...
	/**
	 * @return the minimum value supported by the scrollbar (usually zero).
	 * @see #setMinimum
	 */
	 ...
	 * @beaninfo preferred: true bound: true description: The scrollbar's unit
	 *           increment.
	 */
	 ...
	 * @beaninfo preferred: true bound: true description: The scrollbar's block
	 *           increment.
	 */
	 ...
	 * In many look and feel implementations of scrollbars, the size of the scrollbar "knob"
	 * or "thumb" is proportional to the extent.
	 * 
	 ...
	 * @beaninfo preferred: true description: The amount of the view that is
	 *           currently visible.
	 */
	...
	public static void main(String[] args) {
		...
		LabelScrollbar ls = new LabelScrollbar(1, 10); // Initialize the
														// component.
		ls.setText("Projection #");
		frame.getContentPane().add(ls); // Place the component on the
										// application
